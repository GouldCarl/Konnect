...
async fn handle_move_connected(
    ...
    // Edit the exact text read above, so the one write below can refuse a
    // file that changed underneath this call.
    let mut sch = cse::Schematic::from_source(&sch_path, content.clone())?;
    ...
    // Junction dots are re-judged from the before/after pin sets in memory and
    // committed in the same single write as the move (upstream #624).
    let candidate = sch.to_source();
    let (candidate, junctions_added, junctions_pruned) =
        reconcile_junctions_for_placement(&content, candidate);
    write_atomic_if_unchanged(&sch_path, &content, &candidate)?;
