...
mod power_symbol_tests {
    ...
    fn schematic_with_a_west_pin() -> (tempfile::TempDir, std::path::PathBuf) {
        // J1 and its pin end (91.44, 100.33) sit on the 1.27 mm grid:
        // add_power_symbol snaps its request to that grid (#662), so an
        // off-grid target pin would be missed before any stub is drawn.
        let dir = tempfile::tempdir().unwrap();
    ...
    /// (91.44, 100.33) along that pin's outward ("left") direction, and joins
    ...
    async fn stub_length_mm_moves_the_symbol_off_the_target_pin() {
        ...
        let sym = sch
            ...
            .find(|s| s.reference() == Some("#PWR001"))
            .expect("power symbol instance");
        assert_eq!(sym.at.x, 88.9, "the symbol itself must sit off the pin");
        assert_eq!(sym.at.y, 100.33);
        assert_eq!(sch.wires.len(), 1, "a stub wire must join the two points");
        let wire = &sch.wires[0];
        assert_eq!(wire.start, (91.44, 100.33));
        assert_eq!(wire.end, (88.9, 100.33));
    }
