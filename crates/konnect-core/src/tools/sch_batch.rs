...
use crate::tool;
use crate::tools::{
    all_pin_endpoints, find_all_symbol_instance_blocks, get_path, opt_f64, opt_str, opt_u32,
...
pub fn tools() -> Vec<ToolDef> {
    vec![
        tool!(
            ...
            "Connect multiple component pins to a named net by adding a short wire stub and a              net label at each pin, same as connect_to_net. Single file read → all stubs and              labels inserted → single file write.              Labels are sheet-local: a sheet instanced N times gets N independent nets.              Use add_power_symbol or a global_label for a rail shared by every instance, and              only a local label for one that must stay per-instance — power symbols and              global labels are one net across all sheets and instances.",
            json!({
                "type": "object",
                "properties": {
