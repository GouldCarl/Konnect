...
pub(crate) fn validate_sheet_instance_state(
    ...
    for symbol in &schematic.symbols {
            ...
            || wrong_reference
        {
            ...
            if missing_current_project {
                // Name what the symbol does record, so the refusal still shows
                // both project identities (#592), then the remedy (#20).
                let mut recorded = instances
                    .iter()
                    .filter_map(|instance| {
                        Some((instance.project.clone()?, instance.path.clone()?))
                    })
                    .collect::<Vec<_>>();
                recorded.sort();
                // The remedy is shared, so it is stated once below (#592).
                missing_any = true;
                reasons.push(format!("observed [{}]", format_paths(&recorded)));
            } else if observed != expected {
                // The expected identity is stated once in the summary below (#592).
    ...
    if missing_any {
        diagnoses.push_str(&format!(
            "; where no '{}' entry exists, open the project in eeschema and save once to add it",
